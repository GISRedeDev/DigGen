...
export function zoomToFeature(e, _map, _layer, _layer_grey, _L1_layer, _year, _month) {
    var layer = e.target;
    var iso3 = layer.feature.properties.iso_a3;
    _map.fitBounds(e.target.getBounds());
    _map.removeLayer(_layer);
    _layer_grey.addTo(_map);
    _L1Layer.load_layer(iso3, _year, _month, _map, _L1_layer);
    if (mdebug === true) {
            console.log("zooming : " + layer.feature.properties.iso_a3);
    }
}
...
export function load_layer(year, month, _map, _layer, json) {

    for (var i = 0; i < json.features.length; i++) {
        json.features[i].properties.su_dif = getRandomArbitrary(1, 10);
      //console.log(json.features[i].properties.su_dif);
    }
   _layer.addData(json);
   _layer.addTo(_map);
}
